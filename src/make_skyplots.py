...
def main():
    ...
    #idx = np.where(xm[:,0] == 26220)[0]
    mask = xm[:,0] > 26220 # where we last left off
    ...
    for i in range(len(uniques)):
        ...
        try: # Wrap it in a try except clause in case one of the error ellipses screws up. Can investigate later if we want
            ...
            print(f'HIP {uniques[i]} caused an issue. Drawing without error ellipses')
            fig, ax = plt.subplots(1,1,figsize=(5,5))
            ...
            ax.set_xlabel(r'$\Delta \alpha$ [mas]')
            ax.set_ylabel(r'$\Delta \delta$ [mas]')
            ax.set_title(f'Hip {uniques[i]} | No Error Ellipses')
